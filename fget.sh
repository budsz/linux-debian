...
GETAUD() {
    if [ -z "$awebmf" -a -n "$am4af" ]; then
        yt-dlp $YTOPT -f $am4af -o "f-aud-$IDX.m4a" "$URL"
        cproc=$?
        if [ "$cproc" -eq 1 ]; then
            ccount=0
        ...
        yt-dlp $YTOPT -f $awebmf -o "f-aud-$IDX.webm" "$URL"
        cproc=$?
        if [ "$cproc" -eq 1 ]; then
            ccount=0
            until [ "$cproc" -eq 0 ] || [ "$ccount" -eq 2 ]
        ...
        yt-dlp $YTOPT -f "ba[ext=webm]" -o "f-aud-$IDX.webm" "$URL"
        cproc=$?
        if [ "$cproc" -eq 1 ]; then
            ccount=0
            until [ "$cproc" -eq 0 ] || [ "$ccount" -eq 2 ]
        ...
        yt-dlp $YTOPT -x --audio-format mp3 -o "f-aud-$IDX.mp3" "$URL"
        cproc=$?
        if [ "$cproc" -eq 1 ]; then
            ccount=0
            until [ "$cproc" -eq 0 ] || [ "$ccount" -eq 2 ]
...
if [ -n "$vs720p" -a -z "$vs12xx" ]; then
    ...
    do
        ...
        ffmpeg $FFOPT -i "f-aud+f-vid-$IDX.mp4" -vn -acodec copy "f-aud-$IDX.m4a"
        echo "Creating f-aud-$IDX.m4a file has been successful."

        ffmpeg $FFOPT -i "f-aud+f-vid-$IDX.mp4" -c:v copy -an "f-vid-$IDX.mp4"
...
# Remove ${_tmpfile}, f-aud+f-vid files.
rm -f ${_tmpfile} f-aud+f-vid-*
