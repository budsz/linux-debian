...
if [ -n "$vs720p" -a -z "$vs12xx" ]; then
    ...
    do
        ...
        yt-dlp $YTOPT -f $vr720p -o "$UUIDG.mp4" "$URL"
        cproc=$?
        if [ "$cproc" -eq 1 ]; then
            ccount=0
            until [ "$cproc" -eq 0 ] || [ "$ccount" -eq 2 ]
            do
                yt-dlp $YTOPT -f $vr720p -o "$UUIDG.mp4" $STOPT "$URL"
                cproc=$?
                ccount=$((ccount + 1))
            done
        fi
    ...
    do
        ...
        yt-dlp $YTOPT -f $vr12xx -o "d-aud+d-vid-$UUIDG.mp4" "$URL"
        cproc=$?
        if [ "$cproc" -eq 1 ]; then
            ccount=0
            until [ "$cproc" -eq 0 ] || [ "$ccount" -eq 2 ]
            do
                yt-dlp $YTOPT -f $vr12xx -o "d-aud+d-vid-$UUIDG.mp4" $STOPT "$URL"
                cproc=$?
                ccount=$((ccount + 1))
            done
        fi
...
## 480p (854x480).
    ...
    do
        GETAUD
        yt-dlp $YTOPS -o "$UUIDG" --write-subs --sub-langs "$sublangs" --skip-download "$URL"
        yt-dlp $YTOPT -f $vr480p -o "$UUIDG.mp4" $STOPT "$URL"
        cproc=$?
        if [ "$cproc" -eq 1 ]; then
            ccount=0
            until [ "$cproc" -eq 0 ] || [ "$ccount" -eq 2 ]
    ...
    do
        yt-dlp $YTOPS -o "d-aud+d-vid-$UUIDG" --write-subs --sub-langs "$sublangs" --skip-download "$URL"
        yt-dlp $YTOPT -f $vr8xx -o "d-aud+d-vid-$UUIDG.mp4" $STOPT "$URL"
        cproc=$?
        if [ "$cproc" -eq 1 ]; then
            ccount=0
            until [ "$cproc" -eq 0 ] || [ "$ccount" -eq 2 ]
...
## 360p (640x360).
    ...
    do
        GETAUD
        yt-dlp $YTOPS -o "$UUIDG" --write-subs --sub-langs "$sublangs" --skip-download "$URL"
        yt-dlp $YTOPT -f $vr360p -o "$UUIDG.mp4" $STOPT "$URL"
        cproc=$?
        if [ "$cproc" -eq 1 ]; then
            ccount=0
            until [ "$cproc" -eq 0 ] || [ "$ccount" -eq 2 ]
    ...
    do
        yt-dlp $YTOPS -o "d-aud+d-vid-$UUIDG" --write-subs --sub-langs "$sublangs" --skip-download "$URL"
        yt-dlp $YTOPT -f $vr6xx -o "d-aud+d-vid-$UUIDG.mp4" $STOPT "$URL"
        cproc=$?
        if [ "$cproc" -eq 1 ]; then
            ccount=0
            until [ "$cproc" -eq 0 ] || [ "$ccount" -eq 2 ]
...
## 240p (384x288).
    ...
    do
        GETAUD
        yt-dlp $YTOPS -o "$UUIDG" --write-subs --sub-langs "$sublangs" --skip-download "$URL"
        yt-dlp $YTOPT -f $vr240p -o "$UUIDG.mp4" $STOPT "$URL"
        cproc=$?
        if [ "$cproc" -eq 1 ]; then
            ccount=0
            until [ "$cproc" -eq 0 ] || [ "$ccount" -eq 2 ]
