...
if [ -n "$vs720p" -a -z "$vs12xx" ]; then
    ...
    do
        GETAUD
        yt-dlp $YTOPS -o "$UUIDG" --write-subs --sub-langs "$sublangs" --skip-download "$URL"
        yt-dlp $YTOPT -f $dvs720p -o "$UUIDG.mp4" "$URL"
        cproc=$?
        if [ "$cproc" -eq 1 ]; then
            ccount=0
            until [ "$cproc" -eq 0 ] || [ "$ccount" -eq 2 ]
            do
                yt-dlp $YTOPT -f $dvs720p -o "$UUIDG.mp4" $STOPT "$URL"
                cproc=$?
                ccount=$((ccount + 1))
            done
        fi
    ...
    do
        ...
        yt-dlp $YTOPT -f $dv12xx -o "d-aud+d-vid-$UUIDG.mp4" "$URL"
        cproc=$?
        if [ "$cproc" -eq 1 ]; then
            ccount=0
            until [ "$cproc" -eq 0 ] || [ "$ccount" -eq 2 ]
            do
                yt-dlp $YTOPT -f $dv12xx -o "d-aud+d-vid-$UUIDG.mp4" $STOPT "$URL"
                cproc=$?
                ccount=$((ccount + 1))
            done
        fi

        ffmpeg $FFOPT -i "d-aud+d-vid-$UUIDG.mp4" -vn -acodec copy "d-aud-$IDX.m4a"
...
## 480p (854x480).
    ...
    do
        GETAUD
        yt-dlp $YTOPS -o "$UUIDG" --write-subs --sub-langs "$sublangs" --skip-download "$URL"
        yt-dlp $YTOPT -f $dvs480p -o "$UUIDG.mp4" $STOPT "$URL"
        cproc=$?
        if [ "$cproc" -eq 1 ]; then
            ccount=0
            until [ "$cproc" -eq 0 ] || [ "$ccount" -eq 2 ]
    ...
    do
        yt-dlp $YTOPS -o "d-aud+d-vid-$UUIDG" --write-subs --sub-langs "$sublangs" --skip-download "$URL"
        yt-dlp $YTOPT -f $dv8xx -o "d-aud+d-vid-$UUIDG.mp4" $STOPT "$URL"
        cproc=$?
        if [ "$cproc" -eq 1 ]; then
            ccount=0
            until [ "$cproc" -eq 0 ] || [ "$ccount" -eq 2 ]
...
## 360p (640x360).
    ...
    do
        GETAUD
        yt-dlp $YTOPS -o "$UUIDG" --write-subs --sub-langs "$sublangs" --skip-download "$URL"
        yt-dlp $YTOPT -f $dvs360p -o "$UUIDG.mp4" $STOPT "$URL"
        cproc=$?
        if [ "$cproc" -eq 1 ]; then
            ccount=0
            until [ "$cproc" -eq 0 ] || [ "$ccount" -eq 2 ]
    ...
    do
        yt-dlp $YTOPS -o "d-aud+d-vid-$UUIDG" --write-subs --sub-langs "$sublangs" --skip-download "$URL"
        yt-dlp $YTOPT -f $dv6xx -o "d-aud+d-vid-$UUIDG.mp4" $STOPT "$URL"
        cproc=$?
        if [ "$cproc" -eq 1 ]; then
            ccount=0
            until [ "$cproc" -eq 0 ] || [ "$ccount" -eq 2 ]
...
## 240p (384x288).
    ...
    do
        GETAUD
        yt-dlp $YTOPS -o "$UUIDG" --write-subs --sub-langs "$sublangs" --skip-download "$URL"
        yt-dlp $YTOPT -f $dvs240p -o "$UUIDG.mp4" $STOPT "$URL"
        cproc=$?
        if [ "$cproc" -eq 1 ]; then
            ccount=0
            until [ "$cproc" -eq 0 ] || [ "$ccount" -eq 2 ]
    ...
    do
        yt-dlp $YTOPS -o "d-aud+d-vid-$UUIDG" --write-subs --sub-langs "$sublangs" --skip-download "$URL"
        yt-dlp $YTOPT -f $dv3xx -o "d-aud+d-vid-$UUIDG.mp4" $STOPT "$URL"
        cproc=$?
        if [ "$cproc" -eq 1 ]; then
            ccount=0
            until [ "$cproc" -eq 0 ] || [ "$ccount" -eq 2 ]
