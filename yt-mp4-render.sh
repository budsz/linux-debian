...
FL="$(find * -type f -name "d-aud-*")"
if [ -z "$FL" ]; then
    echo "Audio files doesn't exists!"
    return 1
fi

# Main process.
echo "${FL}" | while read -r dafiles
do
    ## Get layout SINGER - TITLE format from audio files.
    dfsfiles="$(echo $dafiles | awk -F ' - ' '{print $1, "-", $2}' | sed 's/\(.*\)d-aud-//;s/\(.*\)f-aud-//')"
    dftfiles="$(echo $dafiles | cut -d '-' -f 3,4,5 | sed 's/\.[^.]*$//')"
    dfofiles="$(echo $dftfiles - ST)"
    ...
    if [ -n "$dfafiles" ] && [ -n "$dfvfiles" ]; then
        ...
        if [ "$wvfiles" -ge 1280 ]; then
            ffmpeg $FFOPT -i "$dfafiles" -i "$dfafiles" -i "$dfvfiles" -i "$ranlogos" \
