...
if [ -n "$vs12xx" ]; then
    ...
    do
        yt-dlp $YTOPT -f $dv10xx -o "$UUIDG.%(ext)s" --write-subs --sub-lang=$sublang $URL
    ...
    do
        yt-dlp $YTOPT -f $dv7xx -o "$UUIDG.%(ext)s" --write-subs --sub-lang=$sublang $URL
    ...
    do
        yt-dlp $YTOPT -f $dv5xx -o "$UUIDG.%(ext)s" --write-subs --sub-lang=$sublang $URL
    ...
    do
        yt-dlp $YTOPT -f $dv4xx -o "$UUIDG.%(ext)s" --write-subs --sub-lang=$sublang $URL
...
#if [ $? -ne "0" ]; then
#    yt-dlp --force-overwrites -f 18 -o "$UUIDG.%(ext)s" --write-subs $URL
#fi

# Checking exist or not for subtitle files.
vttid="$(ls $UUIDG.id*.vtt 2> /dev/null)"
vttjv="$(ls $UUIDG.jv*.vtt 2> /dev/null)"
vtten="$(ls $UUIDG.en*.vtt 2> /dev/null)"
...
if [ -f "$UUIDG.mp4" ] && [ -f "$vttid" ]; then
    ffmpeg -hide_banner -y -i "$UUIDG.mp4" -vf subtitles="$vttid" "vid-$IDX.mp4"

    # Extract audio from video.
    #ffmpeg -hide_banner -y -i "sub-vid-$IDX.mp4" -map 0:a -b:a 192K "aud-$IDX.mp3"

    # Remove all audio from video.
    #ffmpeg -hide_banner -y -i "sub-vid-$IDX.mp4" -c copy -an "vid-noaudio-$IDX.mp4"
elif [ -f "$UUIDG.mp4" ] && [ -f "$vttjv" ]; then
    ffmpeg -hide_banner -y -i "$UUIDG.mp4" -vf subtitles="$vttjv" "vid-$IDX.mp4"
elif [ -f "$UUIDG.mp4" ] && [ -f "$vtten" ]; then
    ffmpeg -hide_banner -y -i "$UUIDG.mp4" -vf subtitles="$vtten" "vid-$IDX.mp4"
elif [ -f "$UUIDG.mp4" ] && [ -f "$vttsu" ]; then
    ffmpeg -hide_banner -y -i "$UUIDG.mp4" -vf subtitles="$vttsu" "vid-$IDX.mp4"
elif [ -f "$UUIDG.mp4" ] && [ -f "$vttms" ]; then
    ffmpeg -hide_banner -y -i "$UUIDG.mp4" -vf subtitles="$vttms" "vid-$IDX.mp4"
elif [ -f "$UUIDG.mp4" ] && [ -f "$vttja" ]; then
    ffmpeg -hide_banner -y -i "$UUIDG.mp4" -vf subtitles="$vttja" "vid-$IDX.mp4"
fi

# Remove ${_tmpfile}, "$UUIDG".mp4 and "$UUIDG".*.vtt.
rm -f $_tmpfile "$UUIDG".mp4 "$UUIDG".*.vtt
